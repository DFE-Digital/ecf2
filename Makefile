...
RG_TAGS={"Product" : "Early Careers Framework v2 (ECF2)"}
REGION=UK South
SERVICE_NAME=ecf2
SERVICE_SHORT=cpdecf2
...
review: test-cluster ## Specify review AKS environment
	...
	$(eval include terraform/global_config/review.sh)
	$(eval export TF_VAR_pull_request_number=-$(PULL_REQUEST_NUMBER))

.PHONY: staging
staging: test-cluster
	$(eval include terraform/global_config/staging.sh)

sandbox: production-cluster
	$(eval include terraform/global_config/sandbox.sh)

migration: production-cluster
	$(eval include terraform/global_config/migration.sh)

production: production-cluster
	$(if $(or ${SKIP_CONFIRM}, ${CONFIRM_PRODUCTION}), , $(error Missing CONFIRM_PRODUCTION=yes))
	$(eval include terraform/global_config/production.sh)

domains:
	$(eval include terraform/global_config/domains.sh)
...
test-cluster:
	$(eval CLUSTER_RESOURCE_GROUP_NAME=s189t1-tsc-ts-rg)
	$(eval CLUSTER_NAME=s189t1-tsc-test-aks)

production-cluster:
	$(eval CLUSTER_RESOURCE_GROUP_NAME=s189p1-tsc-pd-rg)
	$(eval CLUSTER_NAME=s189p1-tsc-production-aks)
...
aks-console: get-cluster-credentials
	...
	kubectl -n ${NAMESPACE} exec -ti --tty deployment/ecf2-${APP_ID}-web -- /bin/sh -c "cd /app && bundle exec rails c"

aks-ssh: get-cluster-credentials
	$(if $(PULL_REQUEST_NUMBER), $(eval export APP_ID=review-$(PULL_REQUEST_NUMBER)) , $(eval export APP_ID=$(CONFIG_LONG)))
	kubectl -n ${NAMESPACE} exec -ti --tty deployment/ecf2-${APP_ID}-web -- /bin/sh
...
action-group-resources: set-azure-account # make env_aks action-group-resources ACTION_GROUP_EMAIL=notificationemail@domain.com . Must be run before setting enable_monitoring=true for each subscription
	...
	az group create -l uksouth -g ${AZURE_RESOURCE_PREFIX}-${SERVICE_SHORT}-mn-rg --tags "Product=Early Careers Framework v2 (ECF2)" "Environment=Test" "Service Offering=Teacher services cloud"
...
aks-download-tmp-file: get-cluster-credentials
	...
	kubectl get pods -n ${NAMESPACE} -l app=ecf2-${APP_ID}-web -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' | xargs -I {} sh -c 'mkdir -p {}/ && kubectl cp ${NAMESPACE}/{}:/app/tmp/${FILENAME} {}/${FILENAME}'
